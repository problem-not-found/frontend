 ...
 */
export const testGetReviews = async (
  exhibitionId = 1,
  pageNum = 1,
  pageSize = 10
) => {
...
export const useInfiniteReviews = (exhibitionId, pageSize = 10) => {
  ...
  const loadMoreReviews = useCallback(async () => {
    ...
    try {
      ...
      // 디버깅: API 응답 데이터 확인
      console.log(
        `useInfiniteReviews - 페이지 ${currentPage} API 응답:`,
        apiResponse
      );
      console.log("useInfiniteReviews - 새로운 리뷰들:", newReviews);
      console.log(
        "useInfiniteReviews - 리뷰 ID들:",
        newReviews.map((r) => r.reviewId)
      );
