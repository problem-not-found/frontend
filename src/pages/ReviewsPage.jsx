...
const ReviewsPage = () => {
  ...
  // 무한스크롤 리뷰 데이터 가져오기
  const { reviews, loading, hasMore, error, loadMoreReviews, totalElements } =
    useInfiniteReviews(exhibitionId, 10);

  // 디버깅: 리뷰 데이터 확인
  console.log("ReviewsPage - 받은 리뷰 데이터:", reviews);
  console.log("ReviewsPage - 리뷰 개수:", reviews.length);
  console.log(
    "ReviewsPage - 리뷰 ID들:",
    ...
    <div className={styles.container} onScroll={handleScroll}>
      ...
      {loading && (
        <div className={styles.loadingContainer}>
          <ClipLoader color="var(--color-main)" size={35} loading={loading} />
          <p>리뷰를 불러오는 중...</p>
        </div>
      )}
