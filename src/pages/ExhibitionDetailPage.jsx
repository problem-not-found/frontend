...
import {
  useExhibitionDetail,
  usePieceImages,
  useParticipantCreators,
  useExhibitionReviewsPreview,
  useExhibitionAuthor,
...
const ExhibitionDetailPage = () => {
  ...
  } = useParticipantCreators(exhibition?.participantIdList);

  // 전시 주최자 정보 가져오기
  const { author, loading: authorLoading } = useExhibitionAuthor(
    exhibition?.userId
  );
  ...
  const {
    ...
    error: reviewsError,
  } = useExhibitionReviewsPreview(exhibitionId);
  ...
  // 공동전시 여부 판단
  const isGroupExhibition =
    exhibition.participantIdList && exhibition.participantIdList.length > 0;
  ...
  return (
    <div className={styles.container}>
      ...
      <ExhibitionCreator
        creatorName={author?.nickname || "전시 주최자"}
        creatorImage={author?.profileImageUrl || "/creator-profile.png"}
        loading={authorLoading}
        creatorId={exhibition?.userId}
      />
